...
/* ------------------------------------------------------------------ */
/*  Invoice Dialog — Full-screen invoice viewer with download/email     */
/* ------------------------------------------------------------------ */
...
export function InvoiceDialog({
  ...
  useEffect(() => {
    ...
    const url = `/api/customer/invoices/${orderId}?format=html&_t=${Date.now()}`
  ...
  const handleDownload = async () => {
    ...
    setSuccess('')
    try {
      const res = await fetch(`/api/customer/invoices/${orderId}?action=download&_t=${Date.now()}`)
  ...
  const handleResendEmail = async () => {
    ...
    try {
      ...
      setSuccess(data.message || 'Invoice sent to your email')
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {invoiceNumber && (
                <p className="text-[11px] font-mono text-gray-400 truncate">{invoiceNumber}</p>
              )}
            </div>
          </div>
        ...
        >
          <div className="flex gap-2 max-w-3xl mx-auto">
            <Button
              ...
              className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white h-11"
            >
              {downloading ? (
          ...
          {customerEmail && (
            <p className="text-center text-[10px] text-gray-400 mt-2">
              Invoice will be sent to <span className="font-medium text-gray-500">{customerEmail}</span>
            </p>
          )}
        </motion.div>
      </motion.div>
...
/* ------------------------------------------------------------------ */
/*  Invoice Button — Small button to open the invoice dialog            */
/* ------------------------------------------------------------------ */
    ...
    <>
      {variant === 'full' && (
        <Button
          ...
          variant="outline"
          className={cn(
            'flex-1 border-emerald-200 dark:border-emerald-800 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 h-11',
            className,
          )}
        >
          <FileText className="h-4 w-4" />
          <span className="ml-1.5 font-semibold text-sm">View Invoice</span>
        </Button>
      )}
      {variant === 'compact' && (
        <button
          onClick={() => setOpen(true)}
          className={cn(
            'flex items-center gap-1.5 text-emerald-600 hover:text-emerald-700 text-xs font-semibold transition-colors px-2 py-1 rounded',
            className,
          )}
        >
      ...
      {variant === 'icon' && (
        <button
          onClick={() => setOpen(true)}
          className={cn(
            'h-8 w-8 flex items-center justify-center rounded-full text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors',
            className,
          )}
          title="View Invoice"
        >
