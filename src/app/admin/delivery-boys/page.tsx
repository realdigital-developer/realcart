...
/* ------------------------------------------------------------------ */
/*  Delivery Boys Content                                               */
/* ------------------------------------------------------------------ */

function DeliveryBoysContent() {
  // Data state
  ...
  const [deletingBoy, setDeletingBoy] = useState<DeliveryBoy | null>(null)
  ...
  /* ---------------------------------------------------------------- */
  /*  Fetch delivery boys from MongoDB                                 */
  /* ---------------------------------------------------------------- */
  ...
  const fetchDeliveryBoys = useCallback(async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to load delivery boys from database' })
    } finally {
  ...
  const totalPages = Math.max(1, Math.ceil(totalDeliveryBoys / itemsPerPage))

  /* ---------------------------------------------------------------- */
  /*  Selection                                                        */
  /* ---------------------------------------------------------------- */

  const allSelected = deliveryBoys.length > 0 &&
    deliveryBoys.every((d) => selectedIds.has(d._id))

  const toggleSelectAll = useCallback(() => {
    if (allSelected) {
      setSelectedIds(new Set())
    } else {
      setSelectedIds(new Set(deliveryBoys.map((d) => d._id)))
    }
  }, [allSelected, deliveryBoys])
  ...
  /* ---------------------------------------------------------------- */
  /*  View delivery boy detail                                         */
  /* ---------------------------------------------------------------- */

  const openView = useCallback((d: DeliveryBoy) => {
    setViewingBoy(d)
    setViewOpen(true)
  }, [])
  ...
  const openDelete = useCallback((d: DeliveryBoy) => {
    setDeletingBoy(d)
    setDeleteOpen(true)
  }, [])
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      setSelectedIds((prev) => {
        const next = new Set(prev)
        next.delete(deletingBoy._id)
        return next
      })
      setDeleteOpen(false)
      setDeletingBoy(null)
      setMessage({ type: 'success', text: `Delivery boy "${deletingBoy.name}" deleted successfully!` })
      fetchDeliveryBoys()
    } catch (err) {
      setMessage({ type: 'error', text: err instanceof Error ? err.message : 'Failed to delete delivery boy' })
    } finally {
      setSubmitting(false)
    }
  }, [deletingBoy, fetchDeliveryBoys])
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        ids.map(id => fetch(`/api/admin/delivery-boys?id=${id}`, { method: 'DELETE' }))
      )
      ...
      setMessage({
        ...
        text: failed > 0
          ? `${count} delivery boy(s) deleted, ${failed} failed`
          : `${count} delivery boy${count === 1 ? '' : 's'} deleted successfully!`,
      })
      fetchDeliveryBoys()
    } catch (err) {
      setMessage({ type: 'error', text: 'Failed to delete delivery boys' })
    } finally {
      setSubmitting(false)
    }
  }, [selectedIds, fetchDeliveryBoys])
  ...
  const filteredDeliveryBoys = useMemo(() => {
    if (dateFilter === 'all') return deliveryBoys
    const now = new Date()
    ...
    >
      ...
      <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-xl font-semibold tracking-tight">Delivery Boys List</h2>
            <p className="text-sm text-muted-foreground mt-0.5">
              Manage your registered delivery boys here.
            </p>
          </div>
          <motion.button
      ...
      <motion.div variants={fadeInUp} className="rounded-xl border border-border/60 bg-card/50 backdrop-blur-sm overflow-hidden">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/40 hover:bg-muted/40">
                ...
                <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Delivery Boys</TableHead>
                <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Phone Number</TableHead>
                <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Availability</TableHead>
                ...
                {filteredDeliveryBoys.length === 0 ? (
                  ...
                  filteredDeliveryBoys.map((boy) => (
                    <DeliveryBoyRow
                      key={boy._id}
                      boy={boy}
                      selected={selectedIds.has(boy._id)}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border/40">
          <p className="text-xs text-muted-foreground">
            {loadingData ? 'Loading...' : (
              <>
                Showing {totalDeliveryBoys === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1}&#8211;
                {Math.min(currentPage * itemsPerPage, totalDeliveryBoys)} of {totalDeliveryBoys} delivery boys
              </>
            )}
          </p>
      ...
      <AdminModal
        ...
        type="view"
        size="md"
        title={viewingBoy?.name || 'Delivery Boy Details'}
        description={viewingBoy ? `${shortId(viewingBoy._id)} \u00b7 ${viewingBoy.role}` : undefined}
      >
        {viewingBoy && (
          <>
            {/* Avatar + Status */}
            <div className="flex items-center gap-4 mb-4">
              <Avatar className="h-14 w-14 bg-orange-100 dark:bg-orange-900/30">
                <AvatarFallback className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 text-lg font-semibold">
                  {getInitials(viewingBoy.name)}
                </AvatarFallback>
              </Avatar>
              <div className="flex flex-col items-start gap-1.5">
                ...
                <Badge
                  className={cn(
                    ...
                    viewingBoy.isAvailable
                      ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20'
                      : 'bg-gray-500/10 text-gray-600 dark:text-gray-400 border-gray-500/20'
                  )}
                >
                  {viewingBoy.isAvailable ? (
                    <><MapPin className="h-2.5 w-2.5" /> Available</>
                  ) : (
              ...
              <div className="grid grid-cols-1 gap-2.5">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatPhone(viewingBoy.mobile)}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{viewingBoy.isAvailable ? 'Available' : 'Unavailable'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium capitalize">{viewingBoy.role?.replace('_', ' ') || 'Delivery Boy'}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatDate(viewingBoy.createdAt)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatDateTime(viewingBoy.lastLoginAt)}</p>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-lg bg-red-500/5 border border-red-500/10">
                    ...
                    <div>
                      <p className="text-sm font-medium text-destructive">
                        {viewingBoy.blockedReason || 'Account blocked'}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Blocked on {formatDateTime(viewingBoy.blockedAt)}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Failed login attempts: {viewingBoy.failedLoginAttempts}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <AdminDeleteModal
        ...
        itemName="delivery boy"
        name={deletingBoy?.name || ''}
        warningText="This action cannot be undone. All delivery boy data will be permanently removed."
...
/* ------------------------------------------------------------------ */
/*  Delivery Boy Row Component                                          */
/* ------------------------------------------------------------------ */

function DeliveryBoyRow({
  boy,
  selected,
  onToggleSelect,
  onView,
  ...
  onView: (d: DeliveryBoy) => void
  onDelete: (d: DeliveryBoy) => void
}) {
  return (
    <motion.tr
    ...
    >
      <TableCell className="pl-4 w-[44px]">
        <Checkbox
          checked={selected}
          onCheckedChange={() => onToggleSelect(boy._id)}
          aria-label={`Select ${boy.name}`}
        />
      </TableCell>
      ...
      <TableCell>
        <div className="flex items-center gap-3">
          <Avatar className="h-9 w-9">
            <AvatarFallback className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 text-xs font-semibold">
      ...
      <TableCell>
        ...
        >
          {boy.isAvailable ? (
            <><MapPin className="h-3 w-3" /> Available</>
          ) : (
      ...
      <TableCell>
        ...
        >
          {boy.status === 'Active' ? 'Active' : boy.status === 'Blocked' ? 'Blocked' : 'Inactive'}
        </Badge>
      </TableCell>
      <TableCell className="text-sm text-muted-foreground">
